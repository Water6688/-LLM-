...
static void print_usage(const char* app_name) {
  std::cout << "Usage: " << app_name << " <serial_port> [id1,id2,id3|start-end|scan]" << std::endl;
  std::cout << "Example: " << app_name << " /dev/ttyUSB0 1,2,3" << std::endl;
  std::cout << "         " << app_name << " /dev/ttyUSB0 1-6" << std::endl;
  std::cout << "         " << app_name << " /dev/ttyUSB0 scan" << std::endl;
}
...
int main(int argc, char* argv[]) {
  ...
  std::cout << "Reading STS3215 servos on " << port_name << std::endl;
  for (const auto id : ids) {
    ...
    const auto model_result = protocol.read_model_number(id);
    if (!model_result) {
      std::cout << "ID " << static_cast<int>(id) << ": model read failed -> " << model_result.error() << std::endl;
      continue;
    }

    const auto model_number = model_result.value();
    const auto model_name_result = get_model_name(model_number);
    std::string model_name = "unknown";
    if (model_name_result) {
      model_name = std::string(model_name_result.value());
    }
    ...
    const auto speed_result = protocol.read_speed(id);
    if (!speed_result) {
      std::cout << "ID " << static_cast<int>(id) << ": speed read failed -> " << speed_result.error() << std::endl;
      continue;
    }

    const auto position_value = position_result.value();
    const auto speed_value = speed_result.value();
    const auto angle = to_angle(position_value);
    const auto speed_angle = to_angle(speed_value);

    std::cout << "ID " << static_cast<int>(id) << ": model=" << model_name
              << " raw_position=" << position_value
              << " angle=" << angle << " deg"
              << " raw_speed=" << speed_value
              << " speed_angle=" << speed_angle << " deg" << std::endl;
  }
